...
export const load = (async () => {
    ...
    const res_comentario = await fetch(`https://www.googleapis.com/youtube/v3/commentThreads?key=AIzaSyC2ceG7RH57om1JYGxQopMfTJ1rs1nHsu8&videoId=UoifAhH2qiA&part=snippet&order=relevance&maxResults=3`)
    let data_comentario = await res_comentario.json()

    return {
        props: {
            data: data_video, data2: data_comentario
        }
    };
}) satisfies PageLoad;